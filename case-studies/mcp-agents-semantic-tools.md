...
- **Procedencia.** Cada respuesta nombra la vista de la que sale el dato, identificada como la fuente del informe semanal oficial. Si el usuario pregunta de dónde viene el número, el modelo ya tiene el dato y no se lo inventa.
...
**Cómo lo aislé, que es lo que vale.** El agente A (misma plataforma, mismo modelo, mismo camino de streaming, pero hablando por HTTP normal y no por MCP) funcionaba perfectamente. Eso descartó de golpe el rate limit, la red, el modelo y la capa de streaming, y dejó al cliente MCP como único sospechoso.
...
**Las suites.** Seis, partidas por esas mismas líneas: enrutado de tools, planificación, resolución de nombres, síntesis de la respuesta y dos bancos de preguntas por dominio. Puntúa un modelo como juez, porque las respuestas son prosa y comparar cadenas solo mediría la redacción. Cada ejecución se guarda con su resultado por caso, así que un cambio de prompt da un número y no una impresión.
